...
> **Note**: macOS does not support CUDA (Apple Silicon and Intel Macs with AMD GPUs are not compatible). GPU support is only available on Linux with NVIDIA GPUs.
...
GPU support is opt-in. Use `-tags=gpu` to enable it:
