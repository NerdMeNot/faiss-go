...
1. **Static Libraries (Default)** - `//go:build !faiss_use_system`
   - Files: `faiss_lib.go`, `prebuilt_*.go`
   - Uses pre-built static libraries in `libs/<platform>/`
   - Platform-specific LDFLAGS in `prebuilt_linux_amd64.go`, etc.
...
│   ├── libfaiss_c.a      # Custom C wrapper
│   ├── build_info.json   # Build metadata
│   └── checksums.txt     # Library checksums
├── linux_arm64/
├── darwin_amd64/
├── darwin_arm64/
└── windows_amd64/
...
**Build info** tracks FAISS version, compiler, optimization flags.
...
Static library builds use platform-specific linker flags in `prebuilt_*.go`:
