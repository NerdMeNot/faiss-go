...
### 2. **Build Static Libraries** (`build-static-libs.yml`)

**Trigger**: Manual dispatch only (`workflow_dispatch`)

**Purpose**: Builds pre-compiled FAISS static libraries for all platforms

**Behavior**:
- ✅ Can ONLY be triggered manually via GitHub UI
- ❌ Does NOT run automatically on any event
- Builds libraries for: Linux (amd64, arm64), macOS (Intel, ARM64), Windows (amd64)
- Creates a pull request with the built libraries

**How to trigger**:
1. Go to Actions tab
2. Select "Build Static Libraries"
3. Click "Run workflow"
4. Specify FAISS version and platforms

### 3. **Build FAISS Amalgamation** (`build-amalgamation.yml`)

**Trigger**: Manual dispatch only (`workflow_dispatch`)

**Purpose**: Builds FAISS amalgamation (single-file compilation)

**Behavior**:
- ✅ Can ONLY be triggered manually via GitHub UI
- ❌ Does NOT run automatically on any event
- Creates amalgamated source files for easier distribution
- Creates a pull request with the amalgamation

**How to trigger**:
1. Go to Actions tab
2. Select "Build FAISS Amalgamation"
3. Click "Run workflow"
4. Specify FAISS version
...
Feature Branch ──────┐
                     ...
                     └─────────┼──> Merge to main
                          ...
                          main branch
                               ...
                               │    └─ Create release
                               │
                               ├──> Manual: Build Static Libs
                               │    └─ Create PR with libs
                               │
                               └──> Manual: Build Amalgamation
                                    └─ Create PR with amalgam
```
